using ChildFund.Web.Features.Checkout;
    ...
    public class PromotionService : IPromotionService
    {
          ...
          ReferenceConverter referenceConverter)
        {
            ...
            foreach (RewardDescription rewardDescription in _promotionEngine.Evaluate(entryLinks, market, marketCurrency, RequestFulfillmentStatus.Fulfilled))
            {
                ...
                foreach (ILineItem lineItem in rewardDescription.Redemptions.Where(x => x.AffectedEntries != null).SelectMany(x => x.AffectedEntries.PriceEntries).Where(x => x != null).Select(x => x.ParentItem).Where(x => !usedCodes.Contains(x.Code)).Where<ILineItem>(x => entryCodes.Contains(x.Code)))
                {
